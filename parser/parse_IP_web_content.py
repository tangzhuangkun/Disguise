...
class ParseIPWebContent:
    ...
    def parse_web_content(self, url):
        ...
        # 请求加上头信息，伪装成浏览器访问User-Agent
        headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; WOW64; rv:23.0) Gecko/20100101 Firefox/23.0'}

        try:
            # 通过requests的get方法访问目标网站，超时限时 2秒
            req = urllib.request.Request(url=url, headers=headers)
            page = urllib.request.urlopen(req, timeout=2)
            # 解析获得的响应对象
            # utf-8格式解码，
            # 解码的时候加上ignore参数是因为解码过程中有一部分貌似不能正常解码，加上该参数之后能跳过该部分
            raw_content = page.read().decode('utf-8', 'ignore')
        ...
        if web_name == 'kuaidaili' or web_name == 'nimadaili' or web_name == '89ip' or web_name == 'kxdaili':
            # 解析获取的HTML数据
            for tr_content in soup.find_all('tbody'):
                for td_content in tr_content.find_all('tr'):
                    ...
                    if web_name == 'nimadaili':
                    ...
                    elif web_name == 'kuaidaili' or web_name == 'kxdaili':
                        # 获取解析后的IP地址
                        ...
                        # 获取解析后的IP地址
                        ip = td_content_list[0].get_text().strip()
                        # 获取解析后的端口号
                        port = td_content_list[1].get_text().strip()
                        # 获取解析后的匿名类型
                        anonymous_type = "高匿"
                        # 获取解析后的协议类型
                        protocol = None
            ...
            # 解析获取的HTML数据
            for table_content in soup.find_all('table', {'width': '100%', 'border': '2px', 'cellspacing': '0px',
                                                         'bordercolor': '#6699ff'}):
            ...
            print(
                'There is no method to deal with \"{}\" website content, or check the \"collect_proxy_Ip.py\" to make sure there is a method to handle \"{}\"  website'.format(
                    web_name, url))


if __name__ == "__main__":
    go = ParseIPWebContent()
    #page_proxy_ip_detail_set = go.parse_web_content('https://www.kuaidaili.com/free/inha/4/')
    #page_proxy_ip_detail_set = go.parse_web_content('https://www.89ip.cn/index_2.html')
    page_proxy_ip_detail_set = go.parse_web_content('http://www.kxdaili.com/dailiip/1/3.html')
